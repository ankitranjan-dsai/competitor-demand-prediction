...
| 08 | Company Similarity Scoring | Similarity tables + heatmaps/network graphs | ✅ Google done → `docs/task-08-company-similarity-methods.md` (team standard) + `members/ankit-google/task-08-similarity-report.md` (16 tables, 8 figures, 474 tests; five metrics against two nulls, **2 of 15 ranks identified**, Google–Meta the only robust pair, trajectory similarity refused) |
...
- **Corrections are recorded, not overwritten.** A task is submitted the moment
  ...
  [`docs/corrections.md`](docs/corrections.md). Six claims have been
  corrected so far — three by Task 05, one by Task 06, one by Task 07, one by
  Task 08 — and each one is checked against its evidence table by
