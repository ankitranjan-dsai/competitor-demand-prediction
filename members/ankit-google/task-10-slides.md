...
- 10 tasks, 138 committed tables, 54 figures, 737 tests.
...
- 9 corrections are on the record, and each one is checked against the committed table it cites.
...
- 737 tests, including one for every registered correction, so the prose cannot drift from a rebuild.
...
- 138 committed tables and 54 figures across 10 tasks, with row-level data git-ignored throughout.
...
- Yes — 9 times, and each one is on the record with the table that overturned it.
...
- Yes — 737 tests, 138 committed tables and 54 figures, and the core modules import no solver library.
